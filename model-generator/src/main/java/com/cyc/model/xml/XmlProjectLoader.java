...
public class XmlProjectLoader {
  ...
  // Public
  public ProjectObj loadProject(File file) throws KbException, Exception {
    return createProjectObj(loadXML(file));
  }

  public ProjectObj loadProject(String filename) throws KbException, Exception {
  ...
  protected ProjectObj createProjectObj(CycModelDescription description) throws KbException, Exception {
  ...
  protected MethodObj createMethodObj(Method method) throws Exception {
    ...
    if (FuncType.TYPE_GET_LIST.equals(funcType)) {
      return new CriteriaMethodObj(method.getPredicate(), KbCollectionFactory.get(method.getCollection()), funcType);
    }
    if (method.getConfigArgPos() == null) {
      return new MethodObj(method.getPredicate(), KbCollectionFactory.get(method.getCollection()), funcType);
    } else {
      return new MethodObj(method.getPredicate(), KbCollectionFactory.get(method.getCollection()), funcType, method.getConfigArgPos());
    }
  }
