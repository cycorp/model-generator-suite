...
public class MethodObj extends ModelObj {
  ...
  private static String PRED = KBPredicateImpl.class.getSimpleName();
  private static String CONTEXT = ContextImpl.class.getSimpleName();
  private static String FUNC = KBFunctionImpl.class.getSimpleName();
  ...
  public MethodObj(String predStr, KBCollection classColOfMethod, FuncType funcType, Integer configArgPos) throws Exception {
    ...
    if (KBPredicateImpl.existsAsType(predStr)) {
      ...
      for (Integer i = 1; i <= pArity; i++) {
        ...
        Collection<KBCollection> ac = p.getArgGenl(i, Constants.uvMt());
        String rel;
        if (ac.isEmpty()) {
          ac = p.getArgIsa(i, Constants.uvMt());
          rel = "isa";
        } else {
          rel = "genls";
        }
        ...
        int argMod = 0;
        if (firstConstraint.isInstanceOf(MAPIConstants.getInstance().ListTypeByMemType)
                && !firstConstraint.isAtomic()){
          argMod = 2;
          // reset firstConstraint
          firstConstraint = firstConstraint.<KBCollection>getArgument(1);
        } else if (firstConstraint.isInstanceOf(MAPIConstants.getInstance().SetOrColType)
                && !firstConstraint.isAtomic()
                && firstConstraint.<KBFunction>getArgument(0).equals(MAPIConstants.getInstance().SetOfTypeFn)) {
          argMod = 3;
          firstConstraint = firstConstraint.<KBCollection>getArgument(1);
        } else if (firstConstraint.equals(MAPIConstants.getInstance().List)) {
            argMod = 2;
            firstConstraint = KBCollectionImpl.findOrCreate("JavaObject");
        } else if (firstConstraint.equals(MAPIConstants.getInstance().SetMath)) {
            argMod = 3;
            firstConstraint = KBCollectionImpl.findOrCreate("JavaObject");
        }
    ...
    } else if (KBFunctionImpl.existsAsType(predStr)) {
      p = KBFunctionImpl.get(predStr);
      log("Analyzing function: " + p);
      
      if (p.isInstanceOf(MAPIConstants.getInstance().VarArityFunc)){
        log("Function: " + p + " is a variable arity function.");
        Collection<KBCollection> argIsaCol = p.<KBCollection>getValues(MAPIConstants.getInstance().argsIsa, 1, 2, Constants.uvMt());
        ...
        for (Integer i = 1; i <= pArity; i++) {
          ...
          if (ac == null || ac.isEmpty()){
            ac = p.getValues(MAPIConstants.getInstance().argsIsa, 1, 2, Constants.uvMt());
          }
      ...
      Collection<KBCollection> resIsaCols = ((KBFunction) p).getResultIsa(Constants.uvMt());
  ...
  public final KBPredicate getPredicate() {
     ...
     */
    try {
      return KBPredicateImpl.get(getCycName());
    } catch (Exception ex) {
  ...
  public String toString() {
    ...
    methodStr = methodStr + ")";
    methodStr = methodStr + " throws ModelAPIException;";
  ...
  public String getMethodBody() throws ModelGeneratorException {
    ...
    if (!this.getfType().equals(FuncType.FACTORY)) {
      ...
      // Case where this is a "getter", there is a return value.
        ...
        if (this.getArguments().isEmpty()) {
          ...
          if (getret.isSObject()) {
            ...
            methodStr = methodStr + "        Context uvmt = " + CONTEXT + ".get(\"UniversalVocabularyMt\");\n";
      ...
      if (!getret.isSObject()) {
        methodStr = methodStr + "        " + getret.getSObjectType() + " temp = " + FUNC + ".get(\"" + this.getCycName() + "\")"
    ...
    methodStr = methodStr + "      throw new ModelAPIException(e.getMessage(), e);\n";
    methodStr = methodStr + "    }\n";
    return methodStr;
  }
