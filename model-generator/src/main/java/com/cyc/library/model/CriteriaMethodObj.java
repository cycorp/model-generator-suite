...
"      throw new ModelAPIException (ex.getMessage(), ex);\n" +
...
"    }\n\n";
    ...
    /*  
     public static Map<Gizmo, List<java.util.Date>> allGizmoObservedToBeOperationalOnDate(Context ctx) throws Exception {
     Predicate p = new Predicate("gizmoObservedToBeOperationalOnDate");

     Map<Gizmo, List<java.util.Date>> allGizmo_dates = new HashMap<Gizmo, List<java.util.Date>>();
    
     String qStr = "(and (isa ?X Gizmo) "
     + "(gizmoObservedToBeOperationalOnDate ?X ?Y))";
    
     Query q = new Query(ctx.toString(), qStr);
     while(q.next()){
     Gizmo thingamabobTemp = new GizmoImpl(ctx, q.get("?X", KBIndividual.class));
     Date dateTemp = q.get("?Y", Date.class);
     if (allGizmo_dates.containsKey(thingamabobTemp)) {
     allGizmo_dates.get(thingamabobTemp).add(dateTemp);
     } else {
     List<java.util.Date>listTemp = new ArrayList<java.util.Date>();
     listTemp.add(dateTemp);
     allGizmo_dates.put(thingamabobTemp, listTemp);
     }
     }
    
     return allGizmo_dates;
     }*/
    return typeCriteria;
  }
